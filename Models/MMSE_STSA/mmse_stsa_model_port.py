# -*- coding: utf-8 -*-
"""
Port and Implementation of MMSE_STSA Model

    @author: eesungkim
    @Github: https://github.com/eesungkim/Speech_Enhancement_MMSE-STSA.git
"""
import os
import math
import librosa
...
class mmse_stsa_model():
    ...
    def mmse_stsa_process(self):
        noisy_test, sr = librosa.load(self.input_file_path)
        print(sr)
        maxPosteriorSNR = 100
        minPosteriorSNR = 1
        NFFT = 256
        ...
        magnitude_noisy_test, phase_noisy_test = self.divide_magphase(stft_noisy_test, power=1)
        ...
        estNoise = self.estnoisem(pSpectrum, hop_length_sample / sr)
        estNoise = estNoise
        ...
        for i in range(nFFT2):
            ...
            if any(V < 1):
                gain[V < 1] = (math.gamma(1.5) * np.sqrt(V[V < 1])) / aPosterioriSNR_frame[V < 1] * np.exp(
                    -1 * V[V < 1] / 2) * ((1 + V[V < 1]) * self.bessel(0, V[V < 1] / 2) + V[V < 1] * self.bessel(1, V[
                    V < 1] / 2))
        ...
        stft_reconstructed_clean = self.merge_magphase(magnitude_estimated_clean, phase_noisy_test)
        signal_reconstructed_clean = librosa.istft(stft_reconstructed_clean, hop_length=hop_length_sample,
                                                   window=winfunc)
        signal_reconstructed_clean = signal_reconstructed_clean.astype('float32')
        print(signal_reconstructed_clean,'stsa')
        return noisy_test, signal_reconstructed_clean
    ...
    def stft(self, x, n_fft=512, win_length=400, hop_length=160, window='hamming'):
        if window == 'hanning':
        ...
        return np.array(
            [np.fft.rfft(window * x[i:i + win_length], n_fft, axis=0) for i in
             range(0, len(x) - win_length, hop_length)])
...
# mmse_stsa = mmse_stsa_model()
# mmse_stsa.set_file_name(filename='/Users/luantianqi/Downloads/datasets_noisy_white_3dB.wav')
# in_arr, out_arr = mmse_stsa.mmse_stsa_process()
